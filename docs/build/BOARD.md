...
| T13 | UI: Gym player (video, gestures, set panel, rest, end/exit sheets, cooldown, Form video screen) + Sprint player | TODO | |
| T14 | UI: Nora chat (patch card, safety card, error/offline/empty) + What Nora knows + Settings (targets, resets, disclaimer) | TODO | |
| T15 | UI: Nutrition + Progress + Session detail + PR entry sheet | TODO | |
| T16 | UI: Onboarding wizard → build → preview/fail → Today | TODO | |
